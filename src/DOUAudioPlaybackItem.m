                                ...
                                UInt32 *actualCount)
{
    ...
    if ([item filePreprocessor] == nil) {
        *actualCount = (UInt32)[[item fileProvider] readIntoBuffer:buffer withRange:NSMakeRange((NSUInteger)inPosition, requestCount)];
    }
    else {
        *actualCount = (UInt32)[[item fileProvider] readIntoBuffer:buffer withRange:NSMakeRange((NSUInteger)inPosition, requestCount)];
        NSData *input = [NSData dataWithBytesNoCopy:buffer
                                             length:*actualCount
                                       freeWhenDone:NO];
...
- (BOOL)_fillMiscProperties
{
    ...
    status = AudioFileGetProperty(_fileID, kAudioFilePropertyAudioDataPacketCount, &size, &audioDataPacketCount);
    if (status != noErr) {
        return NO;
    }
